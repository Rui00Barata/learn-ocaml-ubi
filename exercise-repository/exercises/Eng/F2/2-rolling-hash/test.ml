...
let genRandomLetterString () =
  let length = Random.int 5 + 2 in
  (* size between 2 and 6 *)
  let genChar () = int_of_char 'a' + Random.int 26 in
...
let hash_test =
  ...
  Section
    ( [ Text "Exercise 1: "; Code "solution" ],
      test_function_1_against_solution [%ty: string -> int] "rolling_hash"
        ~sampler:genRandomLetterString ~gen:8
        [ "za"; "aaa"; "hello"; "ocaml" ] )
...
let hash_rec_test =
  ...
  Section
    ( [ Text "Exercise 2: "; Code "solution" ],
      check_recursion "rolling_hash_rec" @@ fun () ->
      test_function_4_against_solution [%ty: string -> int -> int -> int -> int]
        "rolling_hash_rec" ~sampler:genRandomLetterStringRec ~gen:8
        [
          ("za", 0, 0, 1);
          ("aaa", 0, 0, 1);
          ("hello", 0, 0, 1);
