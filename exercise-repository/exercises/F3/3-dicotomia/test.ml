...
let get_value () =
  ...
  if Random.int(10) >= 3 then 
    (vec.(6 + Random.int((Array.length vec) - 8)), vec, 6, (Array.length vec - 1) - 2)
  else 
    (900, vec, 0, Array.length vec) 
...
let test_1 = Section(
  [Text "Testes da função"; Code "binsearch_aux"],
    (check_recursion "binsearch_aux") @@ fun () -> test_function_4_against_solution
    [%ty: 'a -> 'a array -> int -> int -> int]
    "binsearch_aux"
    ~sampler: (get_value) 
    ~gen: 10
    [(12,[|1;2;5;7;12;16;23;33;78|],2,6)])


let testT = Section(
  [Text "Testes da função"; Code "binsearch"],
  test_function_2_against_solution
  [%ty: 'a -> 'a array -> int]
  "binsearch"
  ~sampler: (get_value2)
  ~gen: 10
  [(12,[|1;2;5;7;12;16;23;33;78|])])
