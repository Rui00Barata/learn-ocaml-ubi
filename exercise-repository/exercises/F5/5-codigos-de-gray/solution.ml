...
let gray_code n = 
  ...
  let tamanho = List.length (binary) in 
  let resultado = ref (String.make tamanho '0') in
  let rec aux i = 
    if i = 0 then resultado := (with_nth_char i (replace_opt (List.nth_opt binary i)) !resultado)
    else resultado := (with_nth_char (i) (replace ((List.nth binary (i-1)) lxor (List.nth binary (i)))) !resultado);
    if i >= (tamanho-1) then if n = 0 then "0" else !resultado else aux (succ i) in
  aux 0;;
